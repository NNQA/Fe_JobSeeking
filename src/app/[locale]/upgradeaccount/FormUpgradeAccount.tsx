...
import { actionUpgradeUser } from "./actions";
...
function FormUpgradeAccount({ email, name }: { name: string; email: string }) {
  ...
  async function onSubmit(data: z.infer<typeof formSchema>) {
    ...
    if (result?.status === "ok") {
      ...
      router.push(`/${locale}/supplier/profile/edit`);
    } else {
      toast({
        variant: "destructive",
        title: "Failure",
      ...
      <div className="px-10 py-4">
        <div className="flex flex-col gap-2">
          <h3 className="h-fit flex gap-2">
            {t("title")}
            <p className="text-primary font-medium">{name ?? email}</p>
          </h3>
